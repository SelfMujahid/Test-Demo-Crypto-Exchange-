// =====================================================
// HOME.JS — index.html ONLY (live market price list).
// Depends on common.js being loaded first (getBal, fmtPrice,
...
// Incoming WS price/change values land here the INSTANT they arrive — never
// delayed. Only the actual screen repaint is batched, once per animation
// frame (~60fps, the display's own refresh rate). Nothing faster than that
// is visible to a human eye anyway, so nothing about the price is "slower."
let pendingUpdates = {};
...
async function fetchTop100() {
  try {
    ...
    // CoinGecko fetch isolated in its own try/catch — if it fails (CORS/rate
    // limit), Binance data still renders instead of the whole page erroring out.
...
// the HIGHER market-cap coin, so the wrong logo/name doesn't win by accident.
...
// CoinGecko-derived fields (1h%, mcap, rank, logo) don't update from the
// Binance WS stream, so they'd freeze at page-load values. This refreshes
// them every 60s (safely under CoinGecko's free-tier rate limit).
...
// ===== STEP 2: Binance WebSocket — live prices, zero compromise =====
let ws;
function connectBinanceWS() {
  ...
  ws.onmessage = (event) => {
    try {
      ...
      // The actual price state updates INSTANTLY, the moment the message
      // arrives — this line never waits for anything.
      ...
      // The DOM write is queued; one requestAnimationFrame flushes
      // everything queued so far, in sync with the screen's own repaint.
      pendingUpdates[sym] = {
        price, change,
        dir: (old && old !== price) ? (price > old ? 'up' : 'down') : null,
      };
      scheduleFlush();
    } catch (e) {}
  };
...
function scheduleFlush() {
  ...
  requestAnimationFrame(() => {
    rafScheduled = false;
    const syms = Object.keys(pendingUpdates);
    if (!syms.length) return;
    syms.forEach(sym => {
      const u = pendingUpdates[sym];
      if (u.dir) flashPrice(sym, u.dir);
      updatePriceCell(sym, u.price, u.change);
    });
    pendingUpdates = {};
    updateStats(); // one combined stats refresh per frame, not per message
  });
}
...
function renderList() {
  ...
  listEl.innerHTML = coins.map((c, i) => {
    ...
    // First 15 rows are near the top of the viewport on load, so they load
    // eagerly. The rest only decode/download once scrolled into view —
    // saves bandwidth and speeds up first paint for long lists.
    const loadAttr = i < 15 ? '' : ' loading="lazy"';
    return `
    <div class="coin-row" onclick="goToCoin('${c.sym}')">
      <div class="rank">${c.rank}</div>
      <div class="coin-info">
        <img src="${c.logo}" alt="${c.sym}" class="coin-logo"${loadAttr} onerror="this.outerHTML='<div class=coin-logo-placeholder>${c.sym.slice(0, 2)}</div>'"/>
...
// ===== DEMO BALANCE — home page's display (uses shared getBal() from common.js) =====
